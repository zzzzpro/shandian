                ...
                try
                {
                    var
                        cmd = // "cat /proc/stat | grep  \"cpu\b\" | awk -v total=0 '{$1=\"\";for(i=2;i<=NF;i++){total+=$i};used=$2+$3+$4+$7+$8 }END{print total,used}';"
                            ...
                            + "prev=$(grep 'cpu ' /proc/stat); sleep 1; curr=$(grep 'cpu ' /proc/stat); awk -v pre=\"$prev\" -v cur=\"$curr\" 'BEGIN {split(pre, a); split(cur, b); prev_idle=a[5]; prev_total=a[2]+a[3]+a[4]+a[5]; idle=b[5]; total=b[2]+b[3]+b[4]+b[5]; printf \"%.2f\n\", (1 - (idle-prev_idle)/(total-prev_total)) * 100}'";
                    var result = Bash(cmd);
